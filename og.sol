...
contract Og is ERC721Enumerable, Ownable {
    ...
    event addressPublicSlotsChange(
        address addr,
        uint256 slots,
    ...
    function addWhitelistAddresses(
        ...
        require(
            ...
            "numberOfTokens is higher that MAX PURCHASE limit!"
        );
    ...
    function isWhitelisted(address _address) public view returns (bool) {
        return
            (_whiteList[_address] > 0 || balanceOf(_address) > 0)
                ? true
                : false;
    }
     ...
     */
    function addressPrivateSaleSlots(address _address)
        public
        view
        returns (uint256)
    {
     ...
     * @return uint8 remaining slots
     */
    function addressPublicSaleSlots(address _address)
        public
        view
        returns (uint8)
    {
     ...
     */
    function mintPrivate(uint8 numberOfTokens, uint8[] memory colors)
        public
        payable
    {
        ...
        require(
            numberOfTokens > 0,
            "Number of tokens cannot be lower than, or equal to 0!"
        );
        require(
            totalSupply() + numberOfTokens <= maxTotalSupply,
            "Total Supply limit have reached!"
        );
        require(
            numberOfTokens <= _whiteList[msg.sender],
            "Not enough presale slots to mint tokens!"
        );
        require(
            OG_PRICE * numberOfTokens == msg.value,
            "Ether value sent is not correct!"
        );

        _whiteList[msg.sender] = uint8(
            _whiteList[msg.sender].sub(numberOfTokens)
        );
     ...
     * @param colors is array with Crystal colors
     */
    function mintPublic(uint8 numberOfTokens, uint8[] memory colors)
        public
        payable
    {
        ...
        require(
            numberOfTokens + _publicSlots[msg.sender] <= MAX_PURCHASE,
            "Address limit have reached!"
        );
        require(
            OG_PRICE * numberOfTokens == msg.value,
            "Ether value sent is not correct!"
        ...
        _publicSlots[msg.sender] = uint8(
            _publicSlots[msg.sender].add(numberOfTokens)
        );
        _mintTokens(msg.sender, numberOfTokens, colors);

        payable(withdrawAddress).transfer(msg.value);

        emit addressPublicSlotsChange(
            msg.sender,
    ...
    function _mintTokens(
        ...
        require(!isFreeze, "contract have already frozen!");
        require(
            numberOfTokens == colors.length,
            "Colors array length should be equal to numberOfTokens!"
        );
        ...
        for (uint8 i = 0; i < numberOfTokens; i++) {
            ...
            _setTokenURI(
                tokenId,
                _formatURI(_baseTokenURI, _uint2str(tokenId))
            );
    ...
    function tokenColor(uint256 tokenId) public view virtual returns (uint8) {
        require(
            _exists(tokenId),
            "ERC721URIStorage: Color query for nonexistent token"
        );
     ...
     */
    function _formatURI(string memory basePath, string memory tokenID)
        internal
        pure
        returns (string memory)
    {
