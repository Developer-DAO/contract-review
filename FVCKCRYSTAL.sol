    ...
    ICreatorExtensionTokenURI
{
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AdminControl, ERC721BurnRedeem, IERC165)
        returns (bool)
    {
        return
            interfaceId == type(ICreatorExtensionTokenURI).interfaceId ||
            AdminControl.supportsInterface(interfaceId) ||
            ERC721BurnRedeem.supportsInterface(interfaceId);
    }
    ...
    function purchase(uint256 amount) external payable nonReentrant {
        ...
        require(
            amount <= redemptionRemaining() && amount <= 5,
            "Too many requested"
        );
        require(
            msg.value == amount * 416900000000000000,
    ...
    {
        require(
            creator == _creator && _mintNumbers[tokenId] != 0,
            "Invalid token"
        );
        return
            string(
